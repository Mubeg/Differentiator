...
DEF_DIF(var, MODE_VARL, '\0' , return (DATA == cur_diff || cur_diff == -1) ? NEW(MODE_CNST, 1, nullptr, nullptr) : NEW(MODE_CNST, 0, nullptr, nullptr);, 0;)
...
DEF_DIF(div, MODE_OPER, '/', return NEW(MODE_OPER, '/', NEW(MODE_OPER, '-', NEW(MODE_OPER, '*', dL, cR), NEW(MODE_OPER, '*', cL, dR)), NEW(MODE_OPER, '*', cR, cR));, DATA = L/R*ACCURACY)

DEF_DIF(div, MODE_OPER, '^', return NEW(MODE_OPER, '*', NEW(MODE_OPER, '*', cR, NEW(MODE_OPER, '^', cL, NEW(MODE_OPER, '-', cR, NEW(MODE_CNST, 1, nullptr, nullptr)))), dL);, DATA = pow(((float) L)/ACCURACY, ((float) R)/ACCURACY)*ACCURACY)

DEF_DIF(sin, MODE_FUNC, '\0', return NEW(MODE_OPER, '*', dR, NEW(MODE_FUNC, FUNC(cos), nullptr, cR));, DATA = sin(((float) R)/ACCURACY)*ACCURACY;)

DEF_DIF(cos, MODE_FUNC, '\1', return NEW(MODE_OPER, '-', nullptr, NEW(MODE_OPER, '*', dR, NEW(MODE_FUNC, FUNC(sin), nullptr, cR)));, DATA = cos(((float) R)/ACCURACY)*ACCURACY;)
